...
class FontViewPanel(wx.Panel):
	...
	def __init__(self, parent):
		...
		self.BIR = {
				...
				self.idRegular: {'style': "regular",'label': _("r"), 'instance': None}
				}
	...
	def onBIR( self, e ):
        ## Michael: I made a few changes in the lines down, run FP and see,
        ## the regular button is still exclusive, but the second cick releases the button.
		id=e.GetId()
		if self.BIR[id]['style'] == "regular":
			state = self.BIR[id]['instance'].GetValue()
			# can't have regular with bold/italic
			self.setAllBIRFalse() # switch all off
			self.setOneBIR( id, state )
			ss = ("", "regular|normal")[state]
		else:
			self.setOneBIR( self.idRegular, False )
			ss=""
			for id, dic in self.BIR.iteritems():
				if dic['instance'].GetValue(): ss += "%s%s" % (dic['style']," ") # Builds AND regex (space is and)
			ss = ss[:-1]
